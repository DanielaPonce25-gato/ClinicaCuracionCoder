...
async function getSeguimientoPaciente(req, res) {
    try {
        const { pacienteId } = req.params;
    ...
    try {
        ...
        const doctors = doctorIds.length
            ? await db.collection('users') // busca en la colección de usuarios para obtener los datos 
                .find({ 
                    _id: { 
                        $in: doctorIds.map(id => new ObjectId(id))
                    }
                }) 
                .toArray() 
            : [];

        // crea un mapa para acceder rápido por id
        const doctorsMap = {};

        doctors.forEach(doc => {
            doctorsMap[doc._id.toString()] = doc;
        });
